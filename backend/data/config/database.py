...
MODEL_PATH = 'model/model_config/lstm_model.h5'
...
def create_model(input_shape, seed=42):
    model = Sequential()
    model.add(LSTM(
    ...
    model.add(Dropout(0.2, seed=seed))
    model.add(Dense(25, kernel_initializer=tf.keras.initializers.glorot_uniform(seed=seed)))
    model.add(Dense(1, kernel_initializer=tf.keras.initializers.glorot_uniform(seed=seed)))
...
if os.path.exists(file_path):
    ...
    model_min = create_model(input_shape=(look_back, 1))
    predictions_min = []
    ...
    model_max = create_model(input_shape=(look_back, 1))
